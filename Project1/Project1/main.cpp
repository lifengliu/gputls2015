...
int main(int argc, char *argv[]) {
	...
	for (size_t i = 0; i < num_platforms; i++) {
		...
		for (size_t j = 0; j < device_num[i]; j++) {
			...
			TestLoopC tloopc(env, src, size, size, size * 2);
			tloopc.init_application_data();
			tloopc.assign_GPU_memory();
			tloopc.fill_data_to_gpu();
			tloopc.init_program();
			tloopc.prepare_kernels();
			tloopc.our_execute();
			tloopc.our_dc();
			tloopc.release_GPU_memory();
